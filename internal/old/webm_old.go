...
func NewWebMMuxerOld(w io.Writer, videoTrack, audioTrack *webrtc.TrackRemote) *WebMMuxer {
...
func (m *WebMMuxerOld) writeEBMLHeader() error {
	header := []byte{
...
func (m *WebMMuxerOld) writeSegmentHeader() error {
...
func (m *WebMMuxerOld) writeInfo() error {
	info := &bytes.Buffer{}
...
func (m *WebMMuxerOld) writeTracks() error {
	tracks := &bytes.Buffer{}
...
func (m *WebMMuxerOld) processVideoStream() {
	var nalBuffer []byte
...
func (m *WebMMuxerOld) Stop() error {
...
func (m *WebMMuxerOld) processAudioStream() {
	var consecutiveErrors int
...
func (m *WebMMuxerOld) writeVideoFrame(data []byte, timestamp uint32) error {
...
func (m *WebMMuxerOld) writeAudioFrame(data []byte, timestamp uint32) error {
	// Calculate timecode in milliseconds using synchronized base timestamp
	m.mutex.Lock()
	baseTS := m.baseTimestamp
	m.mutex.Unlock()
...
func (m *WebMMuxerOld) writeSimpleBlock(trackNum uint64, data []byte, timecode uint64, keyframe bool) error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
...
func (m *WebMMuxerOld) startNewCluster(timecode uint64) error {
...
func (m *WebMMuxerOld) writeEBMLElement(w io.Writer, id uint32, data []byte) error {
	// Write ID
...
func (m *WebMMuxerOld) writeEBMLID(w io.Writer, id uint32) error {
	if id <= 0xFF {
...
func (m *WebMMuxerOld) writeVarInt(w io.Writer, n uint64) error {
	if n < 127 {
...
func (m *WebMMuxerOld) encodeUInt(n uint64) []byte {
	buf := make([]byte, 8)
	size := 0
	for i := 7; i >= 0; i-- {
		if n > 0 || size > 0 {
...
func (m *WebMMuxerOld) encodeFloat(f float64) []byte {
	buf := make([]byte, 8)
	binary.BigEndian.PutUint64(buf, *(*uint64)(unsafe.Pointer(&f)))
	return buf
}

func (m *WebMMuxerOld) encodeFloat32(f float32) []byte {
	buf := make([]byte, 4)
