...
func (m *MPEGTSMuxer) Run() {
	...
	// ビデオ・オーディオストリーム処理開始
	if m.videoTrack != nil {
		go m.processVideoStream()
	}
	if m.audioTrack != nil {
		go m.processAudioStream()
	}
...
func (m *MPEGTSMuxer) processVideoStream() {
	var nalBuffer []byte
	...
	for {
		...
		// RTPタイムスタンプ取得
		if m.videoBaseTS == 0 {
			m.videoBaseTS = rtpPacket.Timestamp
		}

		// H264デパケタイズ（既存のロジック活用）
		...
		if nalType >= 1 && nalType <= 23 {
			// Single NAL unit
			m.writeVideoPES(payload, rtpPacket.Timestamp)
		} else if nalType == 28 {
			// FU-A
			if len(payload) < 2 {
				continue
			...
			if isEnd && len(nalBuffer) > 0 {
				m.writeVideoPES(nalBuffer, rtpPacket.Timestamp)
				nalBuffer = nil
			}
		}
	}
...
func (m *MPEGTSMuxer) processAudioStream() {
	for {
		...
		// RTPタイムスタンプ取得
		if m.audioBaseTS == 0 {
			m.audioBaseTS = rtpPacket.Timestamp
		}

		// Opusペイロード
		m.writeAudioPES(rtpPacket.Payload, rtpPacket.Timestamp)
	}
}
